...
class PickCubeRMA(PickCubeEnv):
    ...
    training.
    '''
    def __init__(self, *args, obj_init_rot_z=True, randomized_training=False, 
                 obs_noise=False, ext_disturbance=False,
                 **kwargs):
        ...
        if self.obs_noise:
            ...
            self.proprio_h_scdl = linear_schedule(0, .005, init_step, end_step)
            self.proprio_l_scdl = linear_schedule(0, -.005, init_step, end_step)
            self.pos_h_scdl = linear_schedule(0, .005, init_step, end_step)
            self.pos_l_scdl = linear_schedule(0, -.005, init_step, end_step)
            self.rot_h_scdl = linear_schedule(0, np.pi*10/180, init_step, 
                                                               end_step)
            self.rot_l_scdl = linear_schedule(0, -np.pi*10/180, init_step, 
                                                                end_step)
