...
class PickSingleRMA(PickSingleEnv):
    ...
    def __init__(self, *args, 
        ...
        self.observation_space['prop_act_history'] = spaces.Box(-np.inf, np.inf, 
                                            shape=[50, 39], dtype=np.float32)
            ...
            if self._episode_rng.uniform() < 0.1:
                ...
                if self.auto_dr:
                    if self.eval_env and self.randomized_param == 'force_scale':
                        ...
                        self.force_scale = self._episode_rng.uniform(self.fs_l, self.fs_h)
                else:
    ...
    def _set_model(self, model_id, model_scale):
        ...
        self.obj1_num_id = get_object_id(task_name="PickSingleYCB", 
        ...
        self.obj1_type_num_id = get_object_id(task_name=self.task_name,
                            model_id=model_type, object_list=self.obj_type_list)
...
class PickSingleYCBRMA(PickSingleYCBEnv):
    ...
    def reset(self, seed=None, options=None):
        ...
        if self.auto_dr:
            if self.eval_env and self.randomized_param == 'obj_position':
                ...
                self.pos_noise = self._episode_rng.uniform(*self.dr_params['obj_position'], 3)
            ...
            self.proprio_h = self.proprio_h_scdl(elapsed_steps=self.step_counter)
            self.proprio_l = self.proprio_l_scdl(elapsed_steps=self.step_counter)
            self.proprio_noise = self._episode_rng.uniform(self.proprio_l, 
                                                    self.proprio_h, 
                                                    self.proprio_dim)
    ...
    def _set_model(self, model_id, model_scale):
        ...
        self.obj1_num_id = get_object_id(task_name="PickSingleYCB", 
                                        model_id=self.model_id,
                                        object_list=self.object_list)
        self.obj1_type_num_id = get_object_id(task_name="PickSingleYCB",
